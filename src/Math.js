 ...
 * - f needs to be bound to the class plot! MATH.f.bind(this) as parameter for polygon()
 * @private
 * @param {number} x1        x1 value in the coordinate system
 * @param {number} x3        x3 value in the coordinate system
 */
export function f(x1, x3) // eslint-disable-line no-unused-vars
{
    // x1 and x3 will be used during eval
    let val = eval(this.validFormula) // f needs to be bound to the class plot!
    return val
}
...
export function transform(formula)
{
    ...
    if(formula.indexOf("x2") != -1 || formula.indexOf("x1") == -1 && formula.indexOf("x3") == -1)
        return console.error("formula",formula,"must contain at least one of x1 or x3")
    ...
    while(!complete)
    {
        ...
        if(formula.indexOf("!") != -1) //if threre is a factorial
        {
            ...
            formula = cutl+"factorial("+leftExpr+")"+formulafac[1]
